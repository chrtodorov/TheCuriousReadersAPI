...
public class BookRequestsRepository : IBookRequestsRepository
{
        ...
        PagingParameters bookRequestParameters)
    {
        ...
        if (customer is null) throw new KeyNotFoundException($"Customer with id: {customerId} does not exist");
    ...
    public async Task<BookRequestModel> MakeRequest(BookRequestModel bookRequest)
    {
        ...
        if (book is null) throw new KeyNotFoundException($"Book with id: {bookRequest.BookId} does not exist");
        ...
        if (alreadyRequestedBook)
            throw new AppException(
                $"User with id: {bookRequest.RequestedBy} has already requested book with id: {book.BookId}");
        ...
        if (bookItem is null)
            throw new AppException($"There are no available copies of a book with id: {bookRequest.BookId}");
        var createdRequest =
    ...
    public async Task RejectRequest(Guid bookRequestId)
    {
        ...
        if (bookRequest is null)
            throw new KeyNotFoundException($"Book request with id: {bookRequestId} does not exist");
