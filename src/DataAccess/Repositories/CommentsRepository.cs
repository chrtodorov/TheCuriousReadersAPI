...
public class CommentsRepository : ICommentsRepository
{
    ...
    public async Task<Comment> AddComment(Comment comment)
    {
        ...
        if (!userExists) throw new KeyNotFoundException($"User with id: {comment.UserId} does not exist");

        var bookExists = await _dbContext.Books.AnyAsync(b => b.BookId == comment.BookId);
        if (!bookExists) throw new KeyNotFoundException($"Book with id: {comment.BookId} does not exist");

        var userReadBook =
            await _dbContext.UserBooks.AnyAsync(ub => ub.UserId == comment.UserId && ub.BookId == comment.BookId);
        if (!userReadBook) throw new AppException($"The current user has not read book with id: {comment.BookId}");
    ...
    public async Task<Comment> ApproveComment(Guid commentId)
    {
        ...
        if (commentEntity is null) throw new KeyNotFoundException($"Comment with id: {commentId} does not exist.");
